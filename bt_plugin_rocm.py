...
class RocmSourceIterator(bt2._UserMessageIterator):
    """Iterator for the ROCm source component with progress bar."""

    def __init__(self, config, output_port):
        """Initialize the iterator and progress tracking."""
        ...
        # Progress bar configuration (env controls)
        self._progress_enabled = os.environ.get("ROCPD_PROGRESS", "1") != "0"
        self._progress_bar_width = int(os.environ.get("ROCPD_PROGRESS_WIDTH", "40"))
        self._progress_stream_name = os.environ.get("ROCPD_PROGRESS_STREAM", "stderr").lower()
        self._progress_file = sys.stdout if self._progress_stream_name == "stdout" else sys.stderr
        self._percent_step_non_tty = max(1, int(os.environ.get("ROCPD_PROGRESS_PERCENT_STEP", "10")))
        self._silent_non_tty = os.environ.get("ROCPD_PROGRESS_SILENT_NON_TTY", "0") == "1"
        self._total_events = 0
        self._emitted_events = 0
        self._last_progress_percent = -1.0
        self._start_time = time.monotonic()
        # Detect interactive terminal capability
        try:
            self._is_tty = self._progress_file.isatty()
        except Exception:
            self._is_tty = False
        ...
        # Load all events and then initialize progress bar
        self._load_all_events()
        self._init_progress()

    def _load_all_events(self):
        """Load events from all databases and tally total for progress bar."""
        self._db_events = []
        self._event_indices = []
        total_events = 0
        global_min = None
    ...
    def __next__(self):
        ...
        if not candidates:
            # Complete progress if pending
            if self._progress_enabled and self._total_events and self._emitted_events >= self._total_events:
                self._print_progress(100.0, force=True, done=True)
            raise StopIteration
        ...
        if msg_type == "event":
            ...
            self._update_progress_event_emitted()
            return msg
    ...
    def _init_progress(self):
        if not self._progress_enabled or self._total_events == 0:
            return
        self._print_progress(0.0, force=True)

    def _print_progress(self, percent: float, force: bool = False, done: bool = False):
        if not self._progress_enabled or self._total_events == 0:
            return
        # Decide if we should emit (TTY vs non-TTY)
        if self._is_tty:
            # TTY: update when >=1% or every 200 events or forced
            if (not force and percent < 100.0 and
                percent - self._last_progress_percent < 1.0 and
                (self._emitted_events % 200) != 0):
                return
        else:
            # Non-TTY (log/file): only emit on step boundaries unless forced/done
            rounded = int(percent)
            if not force and not done:
                if (rounded // self._percent_step_non_tty) == (int(self._last_progress_percent) // self._percent_step_non_tty):
                    return
                if self._silent_non_tty:
                    return
        filled = min(int((percent / 100.0) * self._progress_bar_width), self._progress_bar_width)
        bar = "#" * filled + "." * (self._progress_bar_width - filled)
        elapsed = time.monotonic() - self._start_time
        rate = (self._emitted_events / elapsed) if elapsed > 0 else 0.0
        # Format line
        line = f"[{bar}] {percent:6.2f}% ({self._emitted_events}/{self._total_events}) {rate:7.1f} ev/s"
        if self._is_tty:
            end_char = '' if not done else '\n'
            print('\r' + line, end=end_char, file=self._progress_file, flush=True)
        else:
            # Non-TTY: each line separate (no carriage return)
            print(line, file=self._progress_file, flush=True)
        if done and self._progress_enabled:
            # Final summary
            total_elapsed = time.monotonic() - self._start_time
            avg_rate = (self._total_events / total_elapsed) if total_elapsed > 0 else 0.0
            summary = f"Conversion complete: {self._total_events} events from {len(self._db_paths)} DB(s) in {total_elapsed:.2f}s ({avg_rate:.1f} ev/s)."
            print(summary, file=self._progress_file, flush=True)
        self._last_progress_percent = percent

    def _update_progress_event_emitted(self):
        if not self._progress_enabled or self._total_events == 0:
            return
        self._emitted_events += 1
        percent = (self._emitted_events / self._total_events) * 100.0
